 ...
 */
 ...
 static int max_ser_init(struct max_ser_priv *priv)
 {
	 ...
	 if (ret)
		 return ret;
	 dev_info(priv->dev,"ser init ok !!\r\n");
 
	 for (i = 0; i < priv->ops->num_phys; i++) {
		 struct max_ser_phy *phy = &priv->phys[i];
 
		 if (!phy->enabled){
			dev_info(priv->dev,"current phy is not enabled, continue!!\r\n");
			continue;
		 }
		 ...
		 if (ret){
			dev_info(priv->dev,"ser init_phy not ok,return !!\r\n");
			return ret;
		 }

		 dev_info(priv->dev,"ser init_phy ok !!\r\n");
	 }
	 ...
	 for (i = 0; i < priv->ops->num_pipes; i++) {
		 ...
		 if (!pipe->enabled){
			dev_info(priv->dev,"current pipe is not enabled, continue!!\r\n");
			continue;
		 }
		 ...
		 if (ret){
			dev_info(priv->dev,"ser init_pipe not ok,return !!\r\n");
			return ret;
		 }
