				       ...
				       u8 power_up_addr, u8 new_addr)
{
	...
	if (ret) {
		dev_err(priv->dev,
			"Failed waiting for serializer with new or old address: %d  %x  %x\n", ret, power_up_addr, new_addr);
		goto err_regmap_exit;
	}
...
static int max_des_init(struct max_des_priv *priv)
{
	...
	for (i = 0; i < priv->ops->num_pipes; i++) {
		...
		dev_err(priv->dev, "pipe %d\n", i);
		if (!pipe->enabled)
			continue;

		dev_err(priv->dev, "pipe %d enabled\n", i);
		if (priv->ops->set_pipe_stream_id) {
			ret = priv->ops->set_pipe_stream_id(priv, pipe, pipe->stream_id);
			if (ret)
				return ret;
		}
...
static int max_des_parse_dt(struct max_des_priv *priv)
{
	...
	ret = device_property_read_u32(priv->dev, "maxim,speed-mode", &val);
	if (!ret && val != 0)
...
int max_des_probe(struct max_des_priv *priv)
{
	...
	if (ret){
		dev_err(priv->dev, "Failed to parse device tree: %d\n", ret);
		return ret;
	}
	...
	if (ret){
		dev_err(priv->dev, "Failed to initialize device: %d\n", ret);
		return ret;
	}
	dev_info(priv->dev, "%s %d\n",__func__, __LINE__);
	...
	if (ret){
		dev_err(priv->dev, "Failed to initialize I2C ATR: %d\n", ret);
		return ret;
	}
	dev_info(priv->dev, "%s %d\n",__func__, __LINE__);
	...
	if (ret){
		dev_err(priv->dev, "Failed to post initialize device: %d\n", ret);
		return ret;
	}
	dev_info(priv->dev, "%s %d\n",__func__, __LINE__);
